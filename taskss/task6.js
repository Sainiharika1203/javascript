...
function generatetable(){
    table+=`<tr><th>id</th> <th> quote</th> <th> author</th> <th> Img </th>`
    data .forEach((item)=> {
        table+=`<tr><td>${item.id} </td> <td >${item.quote}</td> <td>${item.author}</td> <td><img src="${item.imgurl}" height="50" width="50"></td> </tr> `
